  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      ...
      const queryParams = new URLSearchParams(
        Object.entries({ ...filters, limit: pagination.limit, offset: pagination.offset }).map(([key, value]) => [
          key,
          String(value),
        ])
      );
        ...
        return (
          <div className={styles.changes}>
            {Object.entries(detailsObj.changes).map(([field, change]) => {
              const changeRecord = change && typeof change === "object" ? change : {};
              const oldValue = "from" in changeRecord ? String(changeRecord.from ?? "") : "";
              const newValue = "to" in changeRecord ? String(changeRecord.to ?? "") : "";
              return (
                <div key={field} className={styles.change}>
                  <strong>{field}:</strong>
                  <div className={styles.changeValues}>
                    <span className={styles.oldValue}>From: {oldValue || "empty"}</span>
                    <span className={styles.arrow}>→</span>
                    <span className={styles.newValue}>To: {newValue || "empty"}</span>
                  </div>
                </div>
              );
            })}
