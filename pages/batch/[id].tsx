...
export default function Page() {
  ...
  const targetAttendanceDate = useMemo(() => {
    ...
    const attendanceDates: string[] | null = batchData?.attendance
      ? Array.from(new Set(batchData.attendance.map((att) => String(normalizeDateString(att.date)))))
      : null;
  ...
  const generateStudentTableData = () => {
    ...
    if (batchData?.students) {
      ...
      res = batchData.students.map((student) => {
        ...
        studentData.percent = total_count === 0 ? 0 : Math.round((present_count / total_count) * 100);
        return studentData;
      });
    }
    return res;
  ...
  const generateColumnsFromAssignment = () => {
    ...
    if (batchData?.grades) {
      ...
      assignmentNames.forEach((assignmentName) => {
        ...
        options.push(
          <option key={assignmentName} value={assignmentName}>
            {assignmentName}
          </option>
        );
      });
