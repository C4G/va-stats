...
export default async function handler(req, res) {
  try {
    if (req.method === "GET") {
      ...
      const courseName = typeof req.query?.course === "string" ? req.query.course : undefined;
    ...
    if (req.method === "POST") {
      ...
      const allowedRoles = new Set<string>(["ADMINISTRATOR"]);
      if (!session || !allowedRoles.has(String(userRole))) {
