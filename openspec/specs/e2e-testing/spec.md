...
The project MUST provide a repeatable seed operation that creates or updates one active staff user for each role offered by the staff form: `STAFF`, `MANAGEMENT`, `ADMINISTRATOR`, `TELECALLER`, `TRAINER`, and `TRAINERPLUSTELECALLER`. It MUST also create or update one course, one batch referring to that course, one student, and the student's assignment to that batch. The fixture MUST include a verified email/password identity for the designated login user and MUST NOT require sending email.
